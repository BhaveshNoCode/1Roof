  ...
  return (
    <footer className="relative bg-primary text-white pt-16 pb-8 px-4">
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map((s) => (
              <Link key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-full border border-gray-600 flex items-center justify-center text-xs text-gray-400 hover:border-accent hover:text-white hover:bg-accent hover:-translate-y-0.5 transition-all duration-300"
                aria-label={s.label}
              >
                {s.icon}
              </Link>
      ...
      <Link
        ...
        className="group fixed bottom-6 right-6 bg-green-500 hover:bg-green-600 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-lift transition-all duration-300 hover:scale-110 z-50"
        aria-label="WhatsApp"
      >
        <span className="absolute inset-0 rounded-full bg-green-500/60 animate-ping [animation-duration:2.5s]" />
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="relative w-7 h-7">
